...
// code
     string getHint(string secret, string guess) {
        ...
        res += to_string(count);
        res += 'A';
        unordered_map<int, int> mpsec;
        unordered_map<int, int> mpguess;
        for(int i = 0; i<secret.length(); i++)
            mpsec[secret[i]-'0']++;
        for(int i = 0; i<guess.length(); i++)
            mpguess[guess[i]-'0']++;
        count = 0;
        for(auto i = mpsec.begin(); i!=mpsec.end(); i++){
            if(mpguess.find(i->first) != mpguess.end()){
                count += min(i->second, mpguess[i->first]);
                cout<<i->first<<mpguess[i->first];
            }
        }   
        res += to_string(count);
        res += 'B';
